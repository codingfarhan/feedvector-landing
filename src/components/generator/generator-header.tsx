        ...
        <div className="grid grid-cols-2 items-center lg:grid-cols-[1fr_auto_1fr]">
          <div className="flex">
            <div className="flex-shrink-0 flex items-center gap-3">
              {/* <!-- Mobile menu button --> */}
              <button aria-label="Toggle left sidebar" onClick={toggleSidebar} className="rounded-md text-gray-400 lg:hidden">
                {sidebarOpen ? <CloseIcon className="size-6" /> : <MenuIcon className="size-6" />}
              </button>

              <div className="flex items-center">
                <Link href="/">
                  <Image src="/images/logo-black.svg" className="block dark:hidden" width={180} height={30} alt="FeedVector Logo" />
                  <Image src="/images/logo-white.svg" className="hidden dark:block" width={180} height={30} alt="FeedVector Logo" />
                </Link>
              </div>
            </div>
          </div>
